...
data: {"phase": "writing", "content": "council "}
...
const SYSTEM_PROMPT = `You are a helpful council assistant. Answer questions about local digital services accurately and cite your sources when possible.`;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const response = await anthropic.messages.stream({
        ...
        max_tokens: 2048,
        system: 'You are a helpful council assistant.',
        messages: [
          ...history,
          { role: 'user', content: message },
        ],
