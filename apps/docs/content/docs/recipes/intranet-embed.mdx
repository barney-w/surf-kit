...
<script>
  (function () {
    ...
    iframe.src = 'https://assistant.example.com/embed';
...
<script>
  (function () {
    ...
    btn.setAttribute('aria-label', 'Open AI Assistant chat');
...
var iframe = document.querySelector('iframe[title="AI Assistant"]');
...
React.useEffect(() => {
  function handleMessage(event: MessageEvent) {
    // Verify origin
    if (event.origin !== 'https://intranet.example.com') return;
...
- [Custom Branding](/docs/theming/custom-branding) — Match the widget to your brand.
