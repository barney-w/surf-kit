...
import { type ColorMode, ThemeProvider } from '@surf-kit/theme'
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border shrink-0">
        <AgentAvatar agent={{ id: 'surf', label: 'Surf' }} size="sm" />
        ...
        {isEmpty ? (
          ...
          <div className="flex flex-col gap-1">
            {visibleMessages.map((msg) => (
              <MessageBubble key={msg.id} message={msg} showAgent showSources />
            ))}
          </div>
        )}
...
function ThemeSwitcher({
  ...
  return (
    <div
      className="inline-flex rounded-lg border border-border p-0.5 gap-0.5"
      role="radiogroup"
      aria-label="Theme"
    >
    ...
    <ThemeProvider colorMode={colorMode} className="h-full">
      <div className="flex h-full bg-canvas text-text-primary">
        ...
        <div className="flex-1 min-w-0">
          <ShowcaseSections colorMode={colorMode} onColorModeChange={setColorMode} />
        </div>
      </div>
    </ThemeProvider>
  )
