import { AgentResponseView, type ChatMessage, TypewriterText } from '@surf-kit/agent'
...
const BG_IMAGES = [`${base}branding/bg.jpg`, `${base}branding/bg2.jpg`, `${base}branding/bg3.jpg`]
...
function StreamingBubble({
  ...
  const { displayed: displayedContent, isDraining } = useCharacterDrain(streamingContent)
      ...
      <div ref={threadRef} className="flex-1 overflow-y-auto overflow-x-hidden py-6">
        {isEmpty ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-8 text-center h-full anim-fade-up">
            {/* Pulsing icon */}
            <img src={`${base}surf.png`} alt="Surf Kit" className="w-32 h-30 rounded-md" />

            <div className="flex flex-col gap-3">
              <h2 className="font-display text-3xl font-bold text-brand-cream">Hi, I'm Surf.</h2>
          ...
          <div className="anim-fade-in">
            {state.messages.map((msg, i) => {
              ...
              const isLastAssistant = i === state.messages.length - 1 && msg.role === 'assistant'
              ...
              return (
                <MessageBubble
                  key={msg.id}
                  msg={msg}
                  noEntryAnimation={(justStoppedLoading || justStoppedDraining) && isLastAssistant}
                  onFollowUp={(text) => {
