...
function MessageBubble({
  ...
  if (isUser) {
    return (
      ...
      >
        <div className="max-w-[75%] rounded-[18px] rounded-br-[4px] px-4 py-2.5 bg-brand-blue text-brand-cream whitespace-pre-wrap text-sm leading-relaxed">
          {message.content}
        </div>
      </div>
    )
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[88%] rounded-[18px] rounded-tl-[4px] px-4 py-3 bg-brand-dark-panel/70 border border-brand-gold/15 backdrop-blur-[8px]">
        {message.response ? (
          <AgentResponse
